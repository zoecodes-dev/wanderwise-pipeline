...
포트폴리오 데모 단계입니다. 위 확장을 거쳐 현재 서울 약 6,600곳이 동네 약 60곳에 걸쳐 라이브로 동선 생성에 쓰이고 있습니다. 시장·공원에는 "거기서 뭘 할지"(`things_to_do`)까지 채워, 한 동네 안에서도 다양한 하루 동선이 나옵니다. 이후로는 새 도시 추가(`config/<city>.yaml`만 작성하면 동일 구조 재사용), 앱 reveal 인터랙션, 검수 어드민 엔드포인트를 이어갈 계획입니다.
